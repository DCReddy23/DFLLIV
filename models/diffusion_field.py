...
class DiffusionFieldMLP(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            noisy_pixels: Noisy pixel RGB values at each coordinate (B, N, output_channels)
...
class DiffusionFieldModel(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            timesteps: Timestep indices of shape (B,)
            noisy_image: Current noisy image of shape (B, 3, H, W)
        ...
        # Sample noisy pixel values at coordinate locations using grid_sample
        # coords are (B, N, 2) in [0, 1] - transform to [-1, 1] for grid_sample
        grid = coords * 2 - 1  # (B, N, 2) in [-1, 1]
        ...
        noisy_pixels = noisy_pixels.squeeze(2).permute(0, 2, 1)  # (B, N, C)
...
if __name__ == "__main__":
    ...
    noisy_pixels = torch.randn(4, 100, 3)
    ...
    noisy_img = torch.randn(2, 3, 256, 256)
